...
export const convertSortStringToArray = (sortStr: string) => {
  const sortItems = sortStr.split(",").map((sortItem) => {
    const fieldToUseToSort = sortItem.startsWith("-")
      ? sortItem.slice(1) + " DESC"
      : sortItem;

    return fieldToUseToSort;
  });
